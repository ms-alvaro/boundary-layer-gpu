"""
Read and visualize a binary snapshot from the GPU Fortran NS solver (nprocs=1).
Scales wall-normal plots by local delta99.

Usage: python3 plot_snapshot.py <snapshot_file> [--fig-dir <dir>]

GPU binary format: same as CPU except each field writes n3-1 z-planes
(excluding the last ghost cell), not the full n3.
"""
...
# Parse arguments
# ============================================================
parser = argparse.ArgumentParser(description='Plot snapshot from GPU BL solver')
...
# ============================================================
# 1. Read binary snapshot (GPU format: n3-1 planes per field)
# ============================================================
def read_int(f, n=1):
    data = f.read(4 * n)
...
def read_double(f, n):
    return np.array(struct.unpack(f'{n}d', f.read(8 * n)))
...
with open(snap_file, 'rb') as f:
    ...
    def read_field_gpu(f):
        """Read a 3D field: header (3 ints) + data.
        GPU (nprocs=1) writes n3-1 z-planes (excluding last ghost)."""
        n1, n2, n3 = struct.unpack('iii', f.read(12))
        n3_actual = n3 - 1
        total = n1 * n2 * n3_actual
        arr = np.frombuffer(f.read(8 * total), dtype=np.float64).copy()
        return arr.reshape((n3_actual, n2, n1)).transpose(2, 1, 0), (n1, n2, n3)

    U, u_shape = read_field_gpu(f)
    V, v_shape = read_field_gpu(f)
    W, w_shape = read_field_gpu(f)

print(f'U: shape={U.shape} (header={u_shape}), range=[{U.min():.4f}, {U.max():.4f}]')
print(f'V: shape={V.shape} (header={v_shape}), range=[{V.min():.4f}, {V.max():.4f}]')
print(f'W: shape={W.shape} (header={w_shape}), range=[{W.min():.4f}, {W.max():.4f}]')
...
# delta99 at each x (z-averaged)
def compute_delta99(U_field, x_arr, yg_arr):
    U_xz_avg = U_field[:, :, :].mean(axis=2)
    delta99 = np.zeros(len(x_arr))
    for i in range(len(x_arr)):
        for j in range(U_field.shape[1] - 1, -1, -1):
            if j < len(yg_arr) and U_xz_avg[i, j] < 0.99 * U_inf:
                if j < U_field.shape[1] - 1:
                    frac = (0.99 * U_inf - U_xz_avg[i, j]) / (U_xz_avg[i, j+1] - U_xz_avg[i, j] + 1e-30)
                    delta99[i] = yg_arr[min(j, len(yg_arr)-1)] + frac * (yg_arr[min(j+1, len(yg_arr)-1)] - yg_arr[min(j, len(yg_arr)-1)])
                else:
                    delta99[i] = yg_arr[min(j, len(yg_arr)-1)]
                break
    return delta99

# U is at (x-faces, y-centers, z-centers) -> use yg for y-axis
delta99 = compute_delta99(U, x, yg[:U.shape[1]])
...
y_targets_d99 = [0.01, 0.1, 0.5, 1.5]
...
# z-coordinates for U: U lives at z-centers = zg
nz_U = U.shape[2]
zg_U = zg[:nz_U] if nz_U <= len(zg) else zg
...
for idx, (yt, yl) in enumerate(zip(y_targets, y_labels)):
    jy = np.argmin(np.abs(yg[:U.shape[1]] - yt))
    ax = axes[idx]
    Uslice = U[:, jy, :].T
    im = ax.pcolormesh(x, zg_U, Uslice, shading='auto', cmap='RdBu_r', vmin=-0.1, vmax=1.1)
...
Uslice = U[:, :, kz_mid].T
yg_U = yg[:U.shape[1]]
im = ax.pcolormesh(x, yg_U / d99_inlet, Uslice, shading='auto', cmap='RdBu_r', vmin=-0.1, vmax=1.1)
ax.plot(x, delta99 / d99_inlet, 'k--', lw=1, label='δ99 (DNS)')
ax.plot(x, delta99_blasius / d99_inlet, 'k:', lw=0.8, alpha=0.5, label='δ99 (Blasius)')
ax.plot(x, delta99_turb / d99_inlet, 'r:', lw=0.8, alpha=0.5, label='δ99 (turbulent)')
ax.set_xlabel('x')
ax.set_ylabel(r'$y / \delta_{99,inlet}$')
ax.set_title(f'U in x-y plane at z = {zg_U[kz_mid]:.5f} (mid-span)')
ax.set_ylim([0, min(yg_U[-1] / d99_inlet, max(5, 3 * delta99.max() / d99_inlet))])
...
for idx, xt in enumerate(x_targets):
    ...
    ax = axes[idx]
    Uslice = U[ix, :, :]
    im = ax.pcolormesh(zg_U, yg_U / d99_local, Uslice, shading='auto', cmap='RdBu_r', vmin=-0.1, vmax=1.1)
    ax.set_title(f'x = {x[ix]:.4f}')
    ax.set_xlabel('z')
    ax.set_ylabel(r'$y / \delta_{99}$')
    ax.set_ylim([0, min(yg_U[-1] / d99_local, 5)])
...
ax2 = ax.twiny()
ax2.set_xlim(ax.get_xlim()[0] * U_inf / nu, ax.get_xlim()[1] * U_inf / nu)
...
ax.plot(x, delta99 / d99_inlet, 'b-', lw=1.5, label='DNS δ99')
ax.plot(x, delta99_blasius / d99_inlet, 'k--', lw=1, label='Blasius δ99')
ax.plot(x, delta99_turb / d99_inlet, 'r--', lw=1, label='Turbulent δ99')
...
print(f'δ99 outlet (DNS, z-avg): {delta99[-1]:.5f}')
