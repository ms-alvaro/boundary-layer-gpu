...
Contains
  ...
  Subroutine compute_alpha_momentum_model(U_,V_,W_)
    ...
    Call apply_periodic_bc_z(Uff,1) ! REPAIR: reference passed id=2 (V-shaped buffers) for the U-shaped Uff -> OOB at nprocs>1
    Call apply_periodic_bc_x(Vff,2)
    Call apply_periodic_bc_z(Vff,2)
    Call apply_periodic_bc_x(Wff,2)
    Call apply_periodic_bc_z(Wff,3) ! REPAIR: reference passed id=1 (U-shaped buffers) for the z-faced Wff
